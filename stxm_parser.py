...
def hdr_get_field(inp_string, field_name):
    input_list = inp_string.split()
    if field_name not in input_list:
        raise Exception('The requested field {} is not present in the file'.format(field_name))
    field_index = input_list.index(field_name) + 3
    curl_count = 1
    output_list = []
    output_list.append('{')
    while curl_count:
        output_element = input_list[field_index]
        if  '{' in output_element:
            curl_count += 1
            field_index += 1
            output_list.append(output_element)
        elif  '}' in output_element:   
            curl_count -= 1
            field_index += 1
            output_list.append(output_element)
        elif (curl_count == 0) or (field_index >= len(input_list)):
            break
        else:
            output_list.append(output_element)
            field_index += 1
...
def hdr_unpack_ScanDefinition(inp_string):
    output_dict = {}
    buffer_list = inp_string.strip('{; }').split(';')
    for element in buffer_list:
        parts_of_element = element.strip(' ').split(' ')
        print(parts_of_element)
        if parts_of_element[0] == 'Label':
            output_dict.update({parts_of_element[0] : parts_of_element[2].strip('"')})
        elif parts_of_element[0] == 'Type':
            output_dict.update({parts_of_element[0] : [parts_of_element[2].strip('"'), parts_of_element[3].strip('"')]})
        elif parts_of_element[0] == 'Flags':
            pass
    print(output_dict)

def hdr_unpack_ScanDefinition_JS(inp_string):
    pass

# Here we take stuff from **file_sdf.py**
...
#----------------------------------------------------------------------
  def parseDataNames(self): #ToDo: Assigning Scan types by DataFlags is prone to errors. Better detect automatically via (HDR.hdr['ScanDefinition']['Regions'][0]) etc.
    ...
    if DataFlag in ['Spectra','Multi-Region Spectra']:
      ...
      DataNames.append(DataNames2)
      print(f"Trying to get the {DataNames} image data file...")
...
#----------------------------------------------------------------------
#----------------------------------------------------------------------
def read(filename, selection=None, JSONstatus=None):
    HDR = HDR_FileParser(filename)
    if JSONstatus:
    ...
    if HDR.hdr['ScanDefinition']['Flags'] == 'Image Stack':
        ...
        x_dist = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['Regions'][selection[0]+1]['PAxis']['Points'][1:] ])
        y_dist = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['Regions'][selection[0]+1]['QAxis']['Points'][1:] ])
        ev = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['StackAxis']['Points'][1:] ])

        n_cols = numpy.array(len(x_dist))
        n_rows = numpy.array(len(y_dist))
        n_ev = numpy.array(len(ev))

        msec = float(HDR.hdr['ScanDefinition']['Dwell'])
        data_dwell = numpy.ones((n_ev))*msec

        imagestack = numpy.empty((n_cols,n_rows,n_ev), numpy.int32)
        for i in range(len(HDR.data_names[selection[0]][selection[1]])):
            try:
                ...
                imagestack[:,:,i] = numpy.nan
        absdata = numpy.empty((n_cols,n_rows, n_ev))

        absdata = numpy.reshape(imagestack, (n_cols,n_rows, n_ev), order='F')

        #fill_h5_struct_from_stk()
    elif HDR.hdr['ScanDefinition']['Flags'] == 'Image':
        x_dist = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['Regions'][1]['PAxis']['Points'][1:] ])
        y_dist = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['Regions'][1]['QAxis']['Points'][1:] ])
        ev = numpy.array([float(i) for i in HDR.hdr['ScanDefinition']['StackAxis']['Points'][1:] ])
        ...
        for i in range(len(HDR.data_names[0][0])):
            try:
                print(f"i is {i}")
                print(f"data_names is {HDR.data_names[0][0][i]}")
        ...
        print("Unsupported file flag..") # ToDo: Add support for multi-region image stacks
        print(HDR.hdr['ScanDefinition']['Flags'])
...
#with open('30529000.hdr') as input_file:
    '''
    file_contents = input_file.read()
    scan_definintion = hdr_get_field(file_contents, 'ScanDefinition')
    hdr_unpack_ScanDefinition(scan_definintion)
    '''
read('30529000.hdr')
#counter_dict = file_structure_JS['Region_0']
